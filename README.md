...
| Tìm kiếm chat | Lọc nhanh transcript đang hiển thị ở thiết bị; nội dung tìm kiếm không đi qua mạng |
...
| Copy chẩn đoán | Sao chép riêng thông tin kỹ thuật để gửi hỗ trợ, không sao chép transcript chat |
| Hướng dẫn Android | Popup từng bước hướng dẫn Orbot, cổng SOCKS5 và đối chiếu fingerprint |
| Tùy chọn cục bộ | Lưu ngôn ngữ, nickname, cỡ chữ và cổng Orbot trong `preferences.json` của app data; không đồng bộ |
| Tin mới nhất | Nút đưa transcript về cuối để xem tin mới sau khi tìm kiếm hoặc cuộn trên Android |
| Bộ đếm tin nhắn | Hiển thị số byte UTF-8 hiện tại so với giới hạn 65.536 byte trước khi gửi; UI chặn gửi khi vượt giới hạn |
| Profile kết nối | Nhớ transport, vai trò, group mode và mức bảo mật gần nhất trong app data; không lưu khóa hoặc session |
| Thời lượng phiên | Hiển thị đồng hồ phiên đang hoạt động để người dùng biết kết nối còn chạy |
| Ẩn chat riêng tư | Ẩn transcript trên màn hình Android mà không xóa log cục bộ hoặc gửi sự kiện mạng |
| Đặt lại profile | Trả transport/role/group/security về mặc định và xóa profile UI cục bộ; không chạm khóa phiên |
| Tự cuộn transcript | Bật/tắt việc tự đưa khung chat về tin mới nhất, phù hợp khi đọc lịch sử trên Android |
| Copy tóm tắt phiên | Sao chép transport/role/group/security/fingerprint để hỗ trợ; không chứa khóa, invite hoặc transcript |
| Tìm kiếm cục bộ | Hiển thị số kết quả dạng `đang hiển thị/tổng số` trong transcript hiện tại |
| Tin chưa đọc | Khi tắt tự cuộn, nút TIN MỚI hiển thị số log mới để người dùng không bỏ lỡ hoạt động |
| Xóa bản nháp | Xóa nhanh nội dung composer và reply target trên thiết bị, không gửi gói rỗng |
| Copy mục gần nhất | Sao chép mục log gần nhất vào clipboard cục bộ; không đưa nội dung mới lên mạng |
| Chống ngủ khi chat | Trên Android giữ màn hình sáng trong phiên đang chạy và khôi phục cài đặt khi dừng |
...
APK debug sẽ nằm trong `bin/`. Trên Android, chọn `LAN trực tiếp` để chat E2EE không cần Tor. Để Join `.onion` bằng Orbot, cài Orbot từ [F-Droid](https://f-droid.org/packages/org.torproject.android/) hoặc [Google Play](https://play.google.com/store/apps/details?id=org.torproject.android), mở Orbot và chờ proxy hoạt động, sau đó chọn `Orbot SOCKS5` trong CloakChat. CloakChat tự thử SOCKS5 tại `127.0.0.1:9050` rồi `127.0.0.1:9150`. Bạn cũng có thể nhập cổng riêng vào ô `Cổng Orbot`/`Orbot port`; ô này được ưu tiên hơn biến môi trường `CLOAKCHAT_ORBOT_SOCKS_PORT`. Nút `KIỂM TRA ORBOT`/`TEST ORBOT` chạy kiểm tra nền để xác nhận listener trước khi Join. Nút `CỠ CHỮ`/`TEXT SIZE` với A−/A/A+ điều chỉnh cỡ chữ cục bộ; `CHẨN ĐOÁN`/`DIAGNOSTICS` hiển thị kết quả kiểm tra gần nhất mà không gửi nội dung chat. `COPY CHẨN ĐOÁN`/`COPY DIAGNOSTICS` chỉ sao chép thông tin kỹ thuật; `HƯỚNG DẪN`/`HELP` mở các bước cấu hình Android. Smoke test không thay thế kiểm thử Orbot trên phần cứng Android thực tế. Các tùy chọn giao diện được lưu cục bộ; khi bắt đầu phiên Android, CloakChat tạm ngăn màn hình ngủ để không làm gián đoạn chat và khôi phục trạng thái khi phiên dừng. Nút `TIN MỚI`/`LATEST` đưa transcript về cuối, còn bộ đếm composer hiển thị byte UTF-8 để người dùng biết kích thước tin nhắn trước khi gửi; UI chặn tin vượt quá 64 KiB trước khi gọi API gửi. Profile kết nối gần nhất và đồng hồ thời lượng phiên chỉ phục vụ UX cục bộ, không chứa khóa mã hóa. `ẨN CHAT`/`HIDE CHAT` che transcript ngay trên thiết bị nhưng không xóa log; `ĐẶT LẠI PROFILE`/`RESET PROFILE` xóa tùy chọn UI đã lưu và đưa cấu hình về mặc định. `TỰ CUỘN`/`AUTO-SCROLL` cho phép tắt việc nhảy về cuối khi đang đọc lịch sử; nút `TIN MỚI`/`LATEST` sẽ đếm hoạt động mới trong lúc đó. Ô tìm kiếm hiển thị số kết quả cục bộ dạng `đang hiển thị/tổng số`. `XÓA SOẠN`/`CLEAR DRAFT` xóa composer và reply target hiện tại trên thiết bị. `COPY CUỐI`/`COPY LAST` sao chép mục log gần nhất vào clipboard cục bộ. `COPY PHIÊN`/`COPY SESSION` chỉ sao chép metadata phiên và fingerprint hiện có, không sao chép khóa, invite hoặc transcript. Hãy mở Orbot, chờ trạng thái đã kết nối rồi mới bấm Bắt đầu trong CloakChat. Bản tích hợp hiện mở Orbot và hỗ trợ Join onion qua SOCKS5; không dùng binary Tor Linux/Windows trong APK.

## Build tự động bằng GitHub Actions

Workflow `.github/workflows/build.yml` được chạy khi push vào `main`, khi push tag dạng `v*`, hoặc khi bấm **Run workflow** trong tab **Actions**. Job Windows đang được tắt có chủ đích theo phạm vi hiện tại; log Windows trước đây dừng vì runner chỉ cung cấp OpenGL 1.1/GDI Generic trong khi Kivy cần tối thiểu OpenGL 2.0. Workflow hiện kiểm tra và đóng gói Linux/Android; Linux chạy thêm GUI smoke test. Windows vẫn không được build theo yêu cầu trước đó. Hai artifact chính là `CloakChat-linux-x86_64` và `CloakChat-android-debug`. Khi chạy từ tag phiên bản, job cuối sẽ gom các artifact và tạo GitHub Release tự động. Windows không còn được build hoặc kiểm thử trong workflow.
